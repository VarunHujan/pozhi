...
const TicketCard = ({ pkg, isSelected, onSelect, index }: TicketCardProps) => {
  return (
    <motion.button
      ...
      onClick={onSelect}
      className={`w-full group relative flex items-stretch rounded-xl border-2 overflow-hidden transition-all duration-300 text-left ${isSelected
    ...
    >
      ...
      <div
        className={`relative w-14 flex-shrink-0 flex items-center justify-center transition-colors duration-300 ${isSelected ? "bg-primary" : "bg-muted"
          }`}
      >
        {/* Top notch */}
        ...
        <Ticket
          className={`w-5 h-5 transition-colors duration-300 ${isSelected ? "text-primary-foreground" : "text-muted-foreground"
            }`}
        />
      </div>
      ...
      <div className="flex-1 flex items-center justify-between p-4 pl-5">
        <div className="flex items-center gap-3">
          <div
            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors duration-200 ${isSelected
        ...
        <motion.span
          ...
          className={`text-lg font-bold font-display tabular-nums ${isSelected ? "text-primary" : "text-foreground"
            }`}
        >
          ₹{pkg.price}
        </motion.span>
